...
const LineWithAnnotationChart = (props) => {
  const { data, withShadow, unit } = props;
  ...
  return (
    <>
      <h6 className="chart-heading text-base uppercase">{heading}</h6>
      <h4 className="chart-value text-2xl mt-2">
        {value} {unit}
      </h4>
      <small className="chart-label text-md uppercase">{label}</small>
      <div className="mt-5">
        <Chart
